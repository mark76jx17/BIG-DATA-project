...
def get_kepler_aggregate_config(df: pd.DataFrame=None,
    ...
    if categories_info is None:
        categories_info = config.get("PALETTES")
    elif categories_info and len(categories_info.keys()) == 1 and "service_count" in categories_info:
        categories_info = config.get("PALETTES")
    ...
    if city:
        if df is not None:
            graphic_config['config']['mapState']['latitude'] = df[df["city"] == city]["lat"].mean()
            graphic_config['config']['mapState']['longitude'] = df[df["city"] == city]["lng"].mean()
            graphic_config['config']['mapState']['zoom'] = 11.5
        ...
        graphic_config['config']['visState']['filters'].append(new_filter)
    ...
    if categories_info != None:
        for i, cat in enumerate(categories_info.keys()):
            ...
            dynamic_multiplier = 35 * (0.4 ** (10 - config.get("H3_RESOLUTION")))
            new_layer = {
                    ...
                    'config': {
                        ...
                        'visConfig': {
                            ...
                            "colorRange": {
                                ...
                                'type': 'sequential',
                                'category': 'Custom',
                                'colors': categories_info[cat]
                            },
                            'enable3d': True,
                            'elevationScale': 5,
                            'sizeRange': [0, int(series.max() * int(dynamic_multiplier))]
                        }
                    },
                    'visualChannels': {
            ...
            graphic_config['config']['visState']['layers'].append(new_layer)
...
def create_category_map(csv_path: pd.DataFrame,
    ...
    graphic_config = get_kepler_aggregate_config(df, categories_info=config.get("PALETTES"), config=config)

    map_cat = KeplerGl(config=graphic_config)
...
def save_map_with_es(kepler_map: KeplerGl,
                     ...
                     config_path: str,
                     csv_data_name: str,
                     explorer_file: str = "explorer.html",
    ...
    explorer_file_path = os.path.join(output_dir, explorer_file)

    categories_json = json.dumps(config.get("SERVICE_CATEGORIES"))
    cities_json = json.dumps([loc['city'] for loc in config.get("LOCATIONS")])
    ...
    html += '</style>\n</head>\n<body>\n<div id="layout">\n'
    html += '<iframe id="map-frame" src="' + filename + '" width="100%" height="100%" style="flex:1;border:none;min-height:100vh" onload="triggerResize(this)"></iframe>\n'
    ...
    html += 'var ES="' + config.get("ELASTICSEARCH_HOST") + '",IX="' + config.get("ELASTICSEARCH_INDEX") + '";\n'
    html += 'var CATS=' + categories_json + ',CITIES=' + cities_json + ';\n'
    html += 'var BASE_MAP="' + filename + '";\n'
    ...
    html += 'tw.innerHTML=t;\n'
    html += '}catch(e){st.className="err";st.textContent="Error: "+e.message+ " (probably ElasticSearch Database is offline)"}\n'
    html += '}\n'
    html += '</script>\n</body>\n</html>'

    with open(explorer_file_path, 'w', encoding='utf-8') as f:
        f.write(html)
...
with open({config_path!r}, mode="r") as f:
...
def make_filtered_map(config, city=None, category=None, min_s=None):
    from visualization import get_kepler_aggregate_config

    csvData = None
    with open({csv_data_name!r}, mode="r") as file:
        csvData = file.read()
    df = pd.read_csv({csv_data_name!r})

    if category == None: category = "service_count"
    if min_s == None: min_s = 0

    graphic_config = get_kepler_aggregate_config(df=df,
                                                categories_info={{category: config.get("PALETTES")[category]}},
                                                city=city,
                                                category=category,
                                                min_s=int(min_s),
                                                config=config)

    m = KeplerGl(config=graphic_config)
    ...
    class Handler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            if parsed.path == "/filter":
                ...
                try:
                    html_bytes = make_filtered_map(config=config, city=city, category=category, min_s=min_s)
                    ...
                    self.send_header("Content-Type", "text/html")
                    self.end_headers()
                    self.wfile.write(f"<html><body style=background:#0f0f23;color:#f87171;display:flex;align-items:center;justify-content:center;height:100vh><h2>Error: {{e}} (probably ElasticSearch Database is offline)</h2></body></html>".encode())
                return
            super().do_GET()
...
print(f"Open http://{{HOST}}:{{PORT}}/{explorer_file} in your browser")
